# Milestone 1b — 长程任务 + Auto-Research + Memory 七动词全量（Step-based，工程 × 学习同步）

- 版本：v1.1（2026-04-22；按 T-P1-5 从原 M1-plan.md 拆出 M1b 子里程碑）
- 作者：dy
- 上位文档：PRD.md §10、TRD.md §4 / §6 / §7 / §8、RnD-Analysis.md §6.2、SPEC.md §5 / §6 / §7 / §8、M1a-plan.md
- 总目标：在 M1a 已证明自研 Core 可用的基础上，完成 `MemoryBackend` 七动词全量（`import_bulk` / `graph` / `lint` 补齐）、`EvaluationRunner` 长程任务能力（SWE-EVO / SlopCodeBench / phoenix-custom 全接入）、Auto-Research v1（≥ 3 轮有效迭代，≥ 2 项 Kept 变更留档）。本子里程碑的北极星是**证明自我优化循环可运作**，允许与 M2 前半部分并行排期。

---

## 0. 启动前提

- **M1a-plan.md 的 §1 DoD 全部成立**（硬前置，特别是 DoD-M1-1 / DoD-M1-3 / DoD-M1-6）。
- `AgentRuntime` / `MemoryBackend` / `ToolSpec` + `PluginRegistry` 三个硬接口已在 M1a Step 7 进入 hard-freeze；M1b 期间禁止 Patch 级接口改动，需变更走 Minor 流程（`spec-change-policy.md`）。
- 学习节点 `F-07 ~ F-15`（M1a 产出）已 `wiki-ingest` 并可被召回。
- 启动冻结版本：`PRD v1.0` / `TRD v1.0` / `SPEC v1.2`。

---

## 1. 完成定义（DoD，状态驱动）

- **DoD-M1-5b**：`MemoryBackend` 七动词全量（`ingest` / `query` / `digest` / `import_bulk` / `graph` / `lint` / `tier`）在 `AKLLMWikiBackend` 全部可用，补齐 M1a 留空的 `import_bulk` / `graph` / `lint`。
- **DoD-M1-7**：长程任务（SWE-EVO / SlopCodeBench 任一 + PhoenixAgent 自定义 ≥ 5 个）完成率 ≥ 80%。
- **DoD-M1-8**：Auto-Research 完成 ≥ 3 轮有效迭代；至少 2 项 Kept 变更在 wiki 留档（含 `experiment-report.md`，双曲线 `CostBreakdown` 字段齐备，SPEC v1.2 §7）。
- **DoD-M1-9b**：学习节点 `F-16 ~ F-22`（见 §4 索引）全部 `wiki-ingest` 并可召回；允许按"能力块"合并（`learning-artifact-rules.md` §4.4）。
- **DoD-M1-10b**：M1b retrospective + interface-backlog 写入 wiki；进入 M2 前三个硬接口再次确认冻结无漂移。
