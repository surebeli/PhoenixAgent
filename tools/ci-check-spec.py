...
# 定义点识别：heading 或加粗首词（可选 "决策" 前缀）出现 ID 视为定义
# 命中形态：
#   "#### FR-01 xxx"
#   "**FR-01**"
#   "- **FR-01**"
...
class SpecChecker:
    ...
    def _collect_definitions(self, text: str) -> set[str]:
        return {m.group(1) for m in DEF_PATTERN.finditer(text)}
        ...
        for rel, info in self.docs.items():
            ...
            for i, line in enumerate(info.text.splitlines(), start=1):
                if SPEC_REF_NO_VER_RE.search(line):
                    # 若本行同时也有 "SPEC v1.x §" 则跳过（精确匹配避免误报）
    ...
    def check_milestone_plan_section0(self) -> None:
        ...
        for rel, info in self.docs.items():
            ...
            missing = []
            # 允许方式：§0 内含 "SPEC v1.x" / "PRD v1.x" / "TRD v1.x" / "RnD-Analysis v1.x"
